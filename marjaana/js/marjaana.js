...
const fci2 = ["affenpinseri", "aidi", "alentejonkoira", "anatolianpaimenkoira ", "appenzellinpaimenkoira", "bernhardinkoira, lyhytkarvainen", "bernhardinkoira, pitkäkarvainen",
    ...
    jQuery('#fcigroup').on("change",  function () {
        ...
        jQuery('#breed').empty();
        if(value == "xrotu"){
                let html = `<option value="Monirotuinen">Monirotuinen</option>`
    ...
    jQuery(document).ready(function () {
        jQuery("select#user_organization").change(function () {
            let value = jQuery("#user_organization").find("option:selected").val();
            if (value == 0) {
                jQuery('#lisaa_uusi_org_label').show();
                jQuery('#lisaa_uusi_org').show();
            }
        })
    });
/*
